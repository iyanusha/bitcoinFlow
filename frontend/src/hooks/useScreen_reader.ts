...
interface Screen_readerState { data: unknown; loading: boolean; error: string | null; initialized: boolean; }

export function useScreen_reader() {
  const [state, setState] = useState<Screen_readerState>({ data: null, loading: false, error: null, initialized: false });
