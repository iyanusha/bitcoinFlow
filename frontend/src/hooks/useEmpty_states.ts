...
interface Empty_statesState { data: unknown; loading: boolean; error: string | null; initialized: boolean; }

export function useEmpty_states() {
  const [state, setState] = useState<Empty_statesState>({ data: null, loading: false, error: null, initialized: false });
