...
interface Canonical_urlsState { data: unknown; loading: boolean; error: string | null; initialized: boolean; }

export function useCanonical_urls() {
  const [state, setState] = useState<Canonical_urlsState>({ data: null, loading: false, error: null, initialized: false });
