...
export function Stat_cards({ children, variant = 'default', loading, className = '' }: Stat_cardsProps) {
  const styles = {
  ...
  return (
    <div className={`stat_cards ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
