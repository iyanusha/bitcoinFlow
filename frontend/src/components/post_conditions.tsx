...
interface Post_conditionsProps {
  children?: ReactNode;
...
export function Post_conditions({ children, variant = 'default', loading, className = '' }: Post_conditionsProps) {
  const styles = {
  ...
  return (
    <div className={`post_conditions ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
