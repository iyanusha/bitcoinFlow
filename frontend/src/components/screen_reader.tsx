...
export function Screen_reader({ children, variant = 'default', loading, className = '' }: Screen_readerProps) {
  const styles = {
  ...
  return (
    <div className={`screen_reader ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
