...
export function Readme_update({ children, variant = 'default', loading, className = '' }: Readme_updateProps) {
  const styles = {
  ...
  return (
    <div className={`readme_update ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
