...
export function High_contrast({ children, variant = 'default', loading, className = '' }: High_contrastProps) {
  const styles = {
  ...
  return (
    <div className={`high_contrast ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
