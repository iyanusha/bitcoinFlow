...
interface Dark_mode_toggleProps {
  children?: ReactNode;
...
export function Dark_mode_toggle({ children, variant = 'default', loading, className = '' }: Dark_mode_toggleProps) {
  const styles = {
  ...
  return (
    <div className={`dark_mode_toggle ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
