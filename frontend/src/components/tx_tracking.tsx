...
export function Tx_tracking({ children, variant = 'default', loading, className = '' }: Tx_trackingProps) {
  const styles = {
  ...
  return (
    <div className={`tx_tracking ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
