...
export function Network_status({ children, variant = 'default', loading, className = '' }: Network_statusProps) {
  const styles = {
  ...
  return (
    <div className={`network_status ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
