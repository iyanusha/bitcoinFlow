...
export function Env_validation({ children, variant = 'default', loading, className = '' }: Env_validationProps) {
  const styles = {
  ...
  return (
    <div className={`env_validation ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
