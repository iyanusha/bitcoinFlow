...
interface Deposit_confirmProps {
  children?: ReactNode;
...
export function Deposit_confirm({ children, variant = 'default', loading, className = '' }: Deposit_confirmProps) {
  const styles = {
  ...
  return (
    <div className={`deposit_confirm ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
