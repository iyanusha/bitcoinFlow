...
export function Test_setup({ children, variant = 'default', loading, className = '' }: Test_setupProps) {
  const styles = {
  ...
  return (
    <div className={`test_setup ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
