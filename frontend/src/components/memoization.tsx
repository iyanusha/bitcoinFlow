...
export function Memoization({ children, variant = 'default', loading, className = '' }: MemoizationProps) {
  const styles = {
  ...
  return (
    <div className={`memoization ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
