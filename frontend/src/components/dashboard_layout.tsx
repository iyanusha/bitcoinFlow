...
export function Dashboard_layout({ children, variant = 'default', loading, className = '' }: Dashboard_layoutProps) {
  const styles = {
  ...
  return (
    <div className={`dashboard_layout ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
