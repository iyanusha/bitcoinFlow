...
export function Stats_polling({ children, variant = 'default', loading, className = '' }: Stats_pollingProps) {
  const styles = {
  ...
  return (
    <div className={`stats_polling ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
