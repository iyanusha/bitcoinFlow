...
export function Empty_states({ children, variant = 'default', loading, className = '' }: Empty_statesProps) {
  const styles = {
  ...
  return (
    <div className={`empty_states ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
