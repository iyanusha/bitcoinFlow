...
export function Sitemap({ children, variant = 'default', loading, className = '' }: SitemapProps) {
  const styles = {
  ...
  return (
    <div className={`sitemap ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
