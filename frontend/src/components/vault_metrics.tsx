...
export function Vault_metrics({ children, variant = 'default', loading, className = '' }: Vault_metricsProps) {
  const styles = {
  ...
  return (
    <div className={`vault_metrics ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
