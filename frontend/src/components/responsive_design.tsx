...
interface Responsive_designProps {
  children?: ReactNode;
...
export function Responsive_design({ children, variant = 'default', loading, className = '' }: Responsive_designProps) {
  const styles = {
  ...
  return (
    <div className={`responsive_design ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
