...
export function Stats_refresh({ children, variant = 'default', loading, className = '' }: Stats_refreshProps) {
  const styles = {
  ...
  return (
    <div className={`stats_refresh ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
