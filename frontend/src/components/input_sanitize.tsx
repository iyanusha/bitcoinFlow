...
export function Input_sanitize({ children, variant = 'default', loading, className = '' }: Input_sanitizeProps) {
  const styles = {
  ...
  return (
    <div className={`input_sanitize ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
