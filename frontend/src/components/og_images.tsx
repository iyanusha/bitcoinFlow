...
export function Og_images({ children, variant = 'default', loading, className = '' }: Og_imagesProps) {
  const styles = {
  ...
  return (
    <div className={`og_images ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
