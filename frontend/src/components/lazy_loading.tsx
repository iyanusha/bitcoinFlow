...
export function Lazy_loading({ children, variant = 'default', loading, className = '' }: Lazy_loadingProps) {
  const styles = {
  ...
  return (
    <div className={`lazy_loading ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
