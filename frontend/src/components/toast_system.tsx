...
export function Toast_system({ children, variant = 'default', loading, className = '' }: Toast_systemProps) {
  const styles = {
  ...
  return (
    <div className={`toast_system ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
