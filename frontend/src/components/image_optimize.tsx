...
export function Image_optimize({ children, variant = 'default', loading, className = '' }: Image_optimizeProps) {
  const styles = {
  ...
  return (
    <div className={`image_optimize ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
