...
export function Mobile_nav({ children, variant = 'default', loading, className = '' }: Mobile_navProps) {
  const styles = {
  ...
  return (
    <div className={`mobile_nav ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
