...
export function Wallet_ux({ children, variant = 'default', loading, className = '' }: Wallet_uxProps) {
  const styles = {
  ...
  return (
    <div className={`wallet_ux ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
