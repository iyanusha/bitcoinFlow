...
export function Api_docs({ children, variant = 'default', loading, className = '' }: Api_docsProps) {
  const styles = {
  ...
  return (
    <div className={`api_docs ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
