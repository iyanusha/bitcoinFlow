...
export function Csp_config({ children, variant = 'default', loading, className = '' }: Csp_configProps) {
  const styles = {
  ...
  return (
    <div className={`csp_config ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
