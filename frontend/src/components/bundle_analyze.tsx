...
export function Bundle_analyze({ children, variant = 'default', loading, className = '' }: Bundle_analyzeProps) {
  const styles = {
  ...
  return (
    <div className={`bundle_analyze ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
