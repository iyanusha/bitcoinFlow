...
interface Dark_mode_applyProps {
  children?: ReactNode;
...
export function Dark_mode_apply({ children, variant = 'default', loading, className = '' }: Dark_mode_applyProps) {
  const styles = {
  ...
  return (
    <div className={`dark_mode_apply ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
