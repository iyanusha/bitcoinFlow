...
export function Env_docs({ children, variant = 'default', loading, className = '' }: Env_docsProps) {
  const styles = {
  ...
  return (
    <div className={`env_docs ${className}`} style={{ ...styles[variant], borderRadius: '12px', padding: '1.25rem' }}>
      {children}
    </div>
  );
}
